...
!----------------------------------------------------------
! Purpose:
!    Perform transformation from Spectral space to 
!       Legendre space
...
! Passed variables:
      Real( 8 ), Intent(In) :: Spectral_Array(:,:,:)  ! Input array in Spectral space
      Real( 8 ), Intent(InOut) :: Legendre_Array(:,:,:)  ! output array in Legendre space
...
! Local variables:
      ...
      Character(24) :: ProcessName = 'Spectral_to_Legendre'
...
!------------------------------ Code Begins ---------------------------
     ...
     &                   Stat = Astat)
                  ...
                  ! receiving array
                  Buffer = Spectral_Array(:,:,Start_Z_recv:End_Z_recv)
     ...
     &                       MPI_Ind(proc), 0, MPI_COMM_WORLD, mpi_err)
            ...
            Do proc = Start_lat_send, End_lat_send
               Legendre_Array(proc,:,:) =
     &             Spectral_Array(proc - Start_lat_send + 1,:,
